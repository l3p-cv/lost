...
export const UsersTable = () => {
  ...
  const editClick = (row) => {
    setIsNewUser(false)
    setSelectedUser(
      usersData.users.filter((el) => el.user_name === row.original.user_name),
    )
    ...
    columnHelper.accessor('roles', {
      ...
      cell: (props) => {
        return props.row.original.roles.map((el) => (
          <RenderBadge key={el.idx} text={el.name} color="success" />
        ))
      },
    }),
    columnHelper.accessor('groups', {
      ...
      cell: (props) => {
        return props.row.original.groups.map((el) => (
          <RenderBadge key={el.idx} text={el.name} color="primary" />
        ))
      },
    }),
