...
export const AnnoTaskStepper = ({
  ...
  const isStepComplete = () => {
    if (currentStep === ANNO_TASK_INFO_STEP) {
      return !isEmpty(annoTaskNodeData.name.trim()) //&&
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case LABEL_TREE_SELECTION_STEP:
        return <SelectTree nodeId={nodeId} availableLabelTrees={availableLabelTrees} />

      case LABEL_SELECTION_STEP:
        return <SelectLabel nodeId={nodeId} availableLabelTrees={availableLabelTrees} />
      ...
      <div className="d-flex justify-content-between mt-3">
        ...
        <div className="ms-auto">
          {canGoToNextStep && (
            <CoreIconButton
              className={
                currentStep === ANNO_TASK_INFO_STEP
                  ? 'step1next'
                  : currentStep === USER_SELECTION_STEP
                    ? 'step2next'
                    : currentStep === LABEL_TREE_SELECTION_STEP
                      ? 'step3next'
                      : currentStep === LABEL_SELECTION_STEP
                        ? 'step4next'
                        : currentStep === STORAGE_SETTINGS_STEP
                          ? 'step5next'
              ...
              toolTip={
                isStepComplete() ? '' : 'Please complete this step before proceeding'
              }
              tTipPlacement="right"
            />
          )}
