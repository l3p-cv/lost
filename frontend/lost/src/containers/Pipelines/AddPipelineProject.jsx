  ...
  const onImportGit = () => {
    ...
    if (gitUrl) {
      ...
      if (
        ...
        projectNames.includes(splittedUrl[splittedUrl.length - 1].split('.git')[0])
      ) {
        Notification.showDecision({
          title: 'The uploaded pipe project already exists, do you want to update it ?',
          option1: {
        ...
        <CRow>
          <CCol sm="12">
            <CollapseCard
              initOpen
              icon={faFileZipper}
              buttonText={' Import / Update pipeline project from .zip File'}
            >
              <CRow>
                <CCol sm="12">
                  <section
                        ...
                        <ul>
                          ...
                          {uploadZipfile ? (
                            <li key={uploadZipfile.path}>
                              {uploadZipfile.path} -{' '}
                              {Number((uploadZipfile.size / 1024).toFixed(2))} KBytes
                            </li>
                          ) : (
                            ''
                          )}
            ...
            <CollapseCard
              icon={faEarth}
              buttonText={'Import / Update pipeline project from a public git repository'}
            >
              <CRow>
                <CCol sm="12">
                  <CForm>
