...
const parseElement = (element, header) => {
    ...
    return {
        abbreviation:
            element[abbreviationIndex] === '' ? null : element[abbreviationIndex],
        color: element[colorIndex] === '' ? null : element[colorIndex],
        description: element[descriptionIndex],
        external_id:
            element[externalIdIndex] === '' ? null : parseInt(element[externalIdIndex]),
        group_id: element[groupIdIndex] === '' ? null : parseInt(element[groupIdIndex]),
        // idx: parseInt(element.idx),
        children: [],
        is_deleted:
            element[isDeletedIndex] === 'True' || element[isDeletedIndex] === 'true',
...
const CreateLabelTree = ({ visLevel }) => {
    ...
    const handleCreateSave = (e) => {
        ...
        if (createLabelName && createLabelDescription) {
            ...
            console.log('CREATE SAVE:', saveData)
            console.log('CREATE VIS LEVEL:', visLevel)
    ...
    useEffect(() => {
        if (lastTree) {
            ...
            while (labelsToAdd.current.length > 0) {
                ...
                parsed.parent_leaf_id =
                    lastTree.idx + parseInt(next[parentLeafIdIndex]) - offset
