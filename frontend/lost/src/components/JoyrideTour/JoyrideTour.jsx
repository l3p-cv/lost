...
import { useConditionalPipelinesPaged } from '../../api/pipeline/pipeline'
...
const JoyrideTour = () => {
  ...
  const pipelineDataSteps = typedSteps
    .map((step, index) =>
      typeof step.target === 'string' && pipelineTargets.includes(step.target)
        ? index
        : -1,
    )
    .filter((index) => index !== -1)

  const requiresPipelineData =
    typedSteps[currentStep] && pipelineDataSteps.includes(currentStep)

  const pipelineQuery = useConditionalPipelinesPaged(
    0,
    10,
    isPipelinePage && requiresPipelineData,
  )
  ...
  const handleJoyrideCallback = ({ action, index, status, type }) => {
    ...
    if (type === EVENTS.STEP_AFTER) {
      ...
      console.log(
        `Joyride step action: ${action}, current index: ${index}, next index: ${nextIndex}, isProcessingPrev: ${isProcessingPrev.current}`,
      )
  ...
  const handleTourStart = (type = 'mainPipeline') => {
    ...
    if (pipelineData == undefined) {
      ...
      const latestPipeline = pipes.reduce(
        (latest, current) =>
