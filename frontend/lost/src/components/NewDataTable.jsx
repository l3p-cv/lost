...
const DataTable = ({
  ...
  const renderPaginationNumbers = (pageCount) => {
    if (pageCount <= 5) {
      ...
      return (
        <>
          {paginationState.pageIndex > 2 && <CPaginationItem>{'...'}</CPaginationItem>}

          {[...Array(renderCount)].map((_, index) => {
            if (index + renderStart === paginationState.pageIndex) {
              return <CPaginationItem active>{index + renderStart + 1}</CPaginationItem>
            }
            return (
              <CPaginationItem
                onClick={() => {
    ...
    return (
      <CCol className=" pt-3 justify-content-between d-flex border-start border-end border-bottom">
        ...
        <CCol className="d-flex align-items-center justify-content-end me-3 mb-3">
          <CDropdown>
            <CDropdownToggle>Show {table.getState().pagination.pageSize}</CDropdownToggle>
            <CDropdownMenu>
                ...
                return (
                  <td>
                    <CInputGroup>
                      ...
                      <CFormInput
                        value={
                          item.getFilterValue() === undefined ? '' : item.getFilterValue()
                        }
                        onChange={(e) => {
          ...
          <div>
            ...
            >
              <CTable striped style={{ marginBottom: '0px' }}>
                  ...
                  {table.getHeaderGroups().map((headerGroup) => (
                    <tr key={headerGroup.id}>
                      {headerGroup.headers.map((header) => (
                        <th
                          className={header.index === 0 ? '' : 'border-start'}
                          key={header.id}
                ...
                <CTableBody>
                  {dataTemp.length === 0 ? (
                    <>{!noEmptyRows && renderEmptyRows(paginationState.pageSize)}</>
                  ) : (
                    <>
                      {renderRows()}
                      {!noEmptyRows &&
                        renderEmptyRows(
                          paginationState.pageSize - table.getRowModel().rows.length,
                        )}
                    </>
                  )}
                </CTableBody>
