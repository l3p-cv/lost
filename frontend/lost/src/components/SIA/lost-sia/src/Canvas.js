...
import cv from "./utils/services/cv";
...
class Canvas extends Component {
  constructor(props) {
    ...
    this.loadOpenCVjs = this.loadOpenCVjs.bind(this)
    this.imageFilterHandler = this.imageFilterHandler.bind(this)
  }
  ...
  async loadOpenCVjs() {
    if (cv) {
      ...
      console.log(this)
      this.props.enableImageFilter()
    }
  }

  async openCvJsImageTransform() {
  ...
  imageFilterHandler(){
    if(this.props.uiConfig.imageFilter.equalizeHist){
      this.openCvJsImageTransform();
    }else{
        this.setState({
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      prevProps.uiConfig.imageFilter.equalizeHist !==
      this.props.uiConfig.imageFilter.equalizeHist
    ) {
      this.imageFilterHandler()
    }
  ...
  deleteAllAnnos() {
    ...
    this.state.annos.forEach(e => {
      if (typeof e.id !== "string") {
        newAnnos.push({ ...e, status: annoStatus.DELETED });
      }
    });
    this.pushHist(
      newAnnos,
  ...
  getAnnoBackendFormat(forBackendPost = false, annos = undefined) {
    ...
    const backendFormat = {
      bBoxes: bAnnos.filter(el => {
        return el.type === "bBox";
      }),
      lines: bAnnos.filter(el => {
        return el.type === "line";
      }),
      points: bAnnos.filter(el => {
        return el.type === "point";
      }),
      polygons: bAnnos.filter(el => {
    ...
    if (mode) {
      ...
      if (mode === modes.DELETED) {
        ...
        newAnno = {
          ...newAnno,
          status:
            anno.status !== annoStatus.NEW ? annoStatus.CHANGED : annoStatus.NEW
        };
      }
    } else {
      newAnno = { ...anno };
  ...
  updateImageSize() {
    ...
    if (this.props.layoutOffset) {
      ...
      maxImgHeight =
        clientHeight -
        container.top -
        this.props.layoutOffset.bottom -
        this.props.layoutOffset.top;
      maxImgWidth =
        container.right - canvasLeft - this.props.layoutOffset.right;
    } else {
        ...
        return (
          <Annotation
            ...
            onAction={(anno, pAction) =>
              this.onAnnoPerformedAction(anno, pAction)
            }
  ...
  render() {
    // console.log("this.state.filteredImage");
    // if(this.state.filteredImage){
    //     console.log(this.state.filteredImage);
    // }
    ...
    return (
      <div ref={this.container}>
        ...
        >
          ...
          />
          <Dimmer active={!this.state.imageLoaded}>
          ...
          {/* <canvas style={{ border: "1px solid black" }} ref={this.testRef} /> */}
          ...
          >
            <g
              transform={`scale(${this.state.svg.scale}) translate(${
                this.state.svg.translateX
              }, ${this.state.svg.translateY})`}
              onMouseOver={() => {
                this.onMouseOver();
              }}
              onMouseUp={e => {
            ...
            >
              <image
                ...
                href={this.state.filteredImage?this.state.filteredImage: this.props.image.data}
                width={this.state.svg.width}
                height={this.state.svg.height}
              />
        ...
        <div style={{ minHeight: this.state.svg.height }} />
      </div>
    );
  }
}
