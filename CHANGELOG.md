...
### Added
### Fixed
- `SiaWrapper.tsx`: Fixed SIA `undo/redo` using stale/deleted DB ids — annotation id mapping is now a synchronous ref, and delete/edit always prefer it over the stale externalId from history snapshots.
...
- sia/endpoint.py: corrected MIME type in thumbnail endpoint to image/jpeg and added missing dbm.close_session() for direction="current"
- db/access.py: corrected anno_task_id/iteration argument order in get_sia_review_prev
...
## [3.4.2] - 2026-07-01
### Fixed
- Fixed dask crashing on initialization (`dask-scheduler` deprecated, -> `dask scheduler`)
...
- Adapted automatic test for bbox creation
### Removed
