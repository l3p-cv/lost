...
def main(args):
    ...
    if root_leaf is None:
        logging.warning(f"LabelTree not present in database! {args.csv_file}")
    else:
        try:
            LabelTree(dbm, root_leaf=root_leaf, logger=logging).delete_tree()
            logging.info(f"Deleted tree with name: {name}")
        except:
            logging.exception("Can not delete label tree. One of the labels is used by a pipeline!")
    dbm.close_session()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--csv_file", nargs="?", action="store", default=None, help="Path to the label tree in csv style."
    )
    parser.add_argument("--name", nargs="?", action="store", default=None, help="Name of the label tree to delete.")
