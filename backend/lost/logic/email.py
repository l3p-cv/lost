    ...
    with app.app_context():
        for ug in pipe.group.users:
            send_email(
                ...
                [ug.user.email],
                render_template(
                    "email/pipeline_finished.html", user=ug.user, pipe=pipe, lost_url=LOST_CONFIG.mail_lost_url
                ),
            )
...
def send_new_user(user, password):
    send_email(
        ...
        [user.email],
        render_template("email/new_user.html", user=user, password=password, lost_url=LOST_CONFIG.mail_lost_url),
    )
