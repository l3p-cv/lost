...
from l3py.model import db_access
from l3py.model import dm
from l3py.model import dtype, state
from l3py.logic.pipeline import pipe_model
from l3py.logic.file_man import FileMan
from l3py.logic import log
from l3py.api import inout
import argparse
import datetime
import traceback
import os
from l3py.logic.config import L3PConfig
import json
import pickle
#import cv2
#from l3py.logic.pipeline.template_import import TemplateQuery
from l3py.api import pipe_element
from l3py.api import annos
...
def report_script_err(pipe_element, task, dbm, msg):
    ...
    pipe_element.state = state.PipeElement.SCRIPT_ERROR.value
    task.state = state.Pipe.ERROR.value
    dbm.add(task)
...
class Script(pipe_element.Element):
    ...
    def __init__(self, pe_id=None):
        ...
        l3pconfig = L3PConfig()
        self.file_man = FileMan(l3pconfig)
        dbm = db_access.ProjectDBMan(l3pconfig)
        if pe_id is None:
    ...
    def static_context(self):
        ...
        '''
        return os.path.join(self._l3pconfig.project_path,
    ...
    def i_am_done(self):
        ...
        if self._pipe_element.is_debug_mode == False:
            self._pipe_element.state = state.PipeElement.FINISHED.value
            self._pipe_element.progress = 100.0
            self._pipe.state = state.Pipe.IN_PROGRESS.value
            ...
            if answer[0].lower() == 'y':
                ...
                self.outp.clean_up()
            self._pipe_man.pipe.state = state.Pipe.IN_PROGRESS.value
            self._dbm.commit()
...
class Output(inout.Output):
    ...
    def add_visual_output(self, img_path=None, html=None):
        ...
        for pe in self._connected_pes:
            if pe.dtype == dtype.PipeElement.VISUALIZATION.value:
                if img_path is not None:
                    rel_path = self._script.file_man.make_path_relative(img_path)
                    vo_type = dtype.VisualOutput.IMAGE.value
                else:
                    rel_path = None
                    vo_type = dtype.VisualOutput.HTML.value
                vis_out = dm.VisualOutput(dtype=vo_type,
                                          img_path=rel_path,
                                          html_string=html,
    ...
    def add_data_export(self, file_path):
        for pe in self._connected_pes:
            if pe.dtype == dtype.PipeElement.DATA_EXPORT.value:
                rel_path = self._script.file_man.make_path_relative(file_path)
                export = dm.DataExport(file_path=rel_path,
                                       result_id=self._result_map[pe.idx],
                                       iteration=self._script._pipe_element.iteration)
    ...
    def request_bba(self, img_path, boxes = [], label_list = [],
        ...
        for pe in self._connected_pes:
            if pe.dtype == dtype.PipeElement.ANNO_TASK.value:
    ...
    def request_mia(self, img_path, sim_class=None, frame_n=None, video_path=None):
        ...
        for pe in self._connected_pes:
            if pe.dtype == dtype.PipeElement.ANNO_TASK.value:
                if pe.anno_task.dtype == dtype.AnnoTask.MIA.value:
                    rel_img_path = self._script.file_man.make_path_relative(img_path)
                    img_anno = annos.Image(anno_task_id=pe.anno_task.idx,
                                          img_path=rel_img_path,
                                          sim_class=sim_class,
