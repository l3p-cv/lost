...
def get_default_script_arguments(script_path):
    try:
        ...
        if arg_dict is None:
            logging.warning(f"Script has no ARGUMENTS: {script_path}!")
            return {}
        else:
            logging.info(f"Found ARGUMENTS: {arg_dict}")
            return arg_dict
    except Exception:
...
def get_default_script_envs(script_path):
    try:
        exec_list = parse_script_constants(script_path, "ENVS", bracket_type="[")
        if exec_list is None:
            logging.warning(f"Script has no ENVS: {script_path}!")
            return []
        else:
            envs = [e.lower() for e in exec_list]
            logging.info(f"Found ENVS: {envs}")
            return envs
    except Exception:
        logging.exception(traceback.format_exc())
...
def get_extra_pip(script_path):
    try:
        exec_list = parse_script_constants(script_path, "EXTRA_PIP", bracket_type="[")
        if exec_list is None:
            logging.warning(f"Script has no EXTRA_PIP: {script_path}!")
            return []
        else:
            # envs = [e.lower() for e in exec_list]
            envs = exec_list
            logging.info(f"Found EXTRA_PIP: {envs}")
            return envs
    except Exception:
        logging.exception(traceback.format_exc())
...
def get_extra_conda(script_path):
    try:
        exec_list = parse_script_constants(script_path, "EXTRA_CONDA", bracket_type="[")
        if exec_list is None:
            logging.warning(f"Script has no EXTRA_PIP: {script_path}!")
            return []
        else:
...
def get_script_args(script_path, arg_name, bracket_type="[", to_lower=True):
    try:
        arg_list = parse_script_constants(script_path, arg_name, bracket_type=bracket_type)
        if arg_list is None:
            logging.warning(f"Script has no {arg_name}: {script_path}!")
            return []
        else:
            if to_lower:
                ...
                args = arg_list
            logging.info(f"Found {arg_name}: {args}")
            return args
    except Exception:
        logging.exception(traceback.format_exc())
...
def get_default_script_resources(script_path):
    try:
        res_list = parse_script_constants(script_path, "RESOURCES", bracket_type="[")
        if res_list is None:
            logging.warning(f"Script has no special RESOURCES requirements: {script_path}!")
            return []
        else:
            res = [e.lower() for e in res_list]
            logging.info(f"Found RESOURCES requirements: {res}")
            return res
    except Exception:
        logging.exception(traceback.format_exc())
