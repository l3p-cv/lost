...
def process_pipeline(pipe: model.Pipe, dbm: access.DBMan):
    ...
    try:
        ...
        pipe_man = cron.PipeEngine(dbm=dbm, pipe=pipe, lostconfig=lostconfig, client=client, logger_name="cron_jobs")
...
class PipeStarter:
    ...
    def create_pe_raw_element(self, pe_n):
        ...
        pipe_element = model.PipeElement(state=state.PipeElement.PENDING, pipe_id=self.pipe.idx, is_debug_mode=False)
        return pipe_element
    ...
    def create_result_links(self, pe_n, result_id=None):
        ...
        if pe_outs:
            for pe_out in pe_outs:
                result_link = model.ResultLink(
                    result_id=result_id, pe_n=self.pe_map.get(pe_n).idx, pe_out=self.pe_map.get(pe_out).idx
                )
                result_links.append(result_link)
        # if last element in pipeline - create one last
        # result link without pe_out
        else:
            result_link = model.ResultLink(result_id=None, pe_n=self.pe_map.get(pe_n).idx)
            result_links.append(result_link)
        return result_links
...
def pause(db_man, pipe_id):
    ...
    if pipe_id is not None:
        ...
        if pipe is not None:
            if (
                pipe.state != state.Pipe.FINISHED
                and pipe.state != state.Pipe.ERROR
                and pipe.state != state.Pipe.PAUSED
                and pipe.state != state.Pipe.PENDING
            ):
