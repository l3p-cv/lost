...
from lost.logic.script import (
    get_default_script_arguments,
    get_default_script_envs,
    get_default_script_resources,
    get_script_args,
)
...
def parse_script(element):
    ...
    """
    script = model.Script(name=element["name"], path=element["path"], description=element["description"])
    return script
...
class PipeImporter:
    def __init__(self, pipe_template_dir, dbm, user_id=None, forTest=False):
        ...
        self.dst_pipe_template_path = os.path.join(
            self.file_man.pipe_path, os.path.basename(self.src_pipe_template_path)
        )
        ...
        for json_path in self.json_files:
            ...
            pipe["name"] = self._namespaced_name(os.path.splitext(os.path.basename(json_path))[0])
            self.pipes.append(pipe)
            # Set name to name of the script file
            for pe in pipe["elements"]:
                if "script" in pe:
                    pe["script"]["name"] = self._namespaced_name(pe["script"]["path"])
    ...
    def start_import(self):
        logging.info("\n\n++++++++++++++++++++++ \n\n")
        logging.info(f"Start pipe project import for: {self.src_pipe_template_path}")
        error_message = ""
        ...
        try:
            ...
            for db_pipe in self.dbm.get_all_pipeline_templates():
                ...
                if db_json["name"].lower() == pipe["name"].lower():
                    ...
                    return error_message
            if not pipe_in_db:
                pipe_temp = model.PipeTemplate(
                    json_template=json.dumps(pipe),
            ...
            logging.exception("Cleanup successful. Removed buggy pipeline.")
            return error_message
    ...
    def remove_pipe_project(self):
        ...
        for pipe in self.pipes:
            if self.remove_pipeline(pipe):
                ...
                logging.info(f"""Pipeline project {self.namespace} was not completely removed 
                    since some pipes are still in use""")
                    ...
                    for pe_j in db_json["elements"]:
                        if "script" in pe_j:
                            script_man.remove_script(
                                self.dbm, os.path.join(self.namespace, os.path.basename(pe_j["script"]["path"]))
                            )
...
class PipeDefChecker:
    ...
    def _check_key(self, key_name, dict_element, types, values=None):
        ...
        if key_name not in dict_element:
            self.logger.error(
                f"No *{key_name}* found! Add a *{key_name}* key "
                f"to the following element:\n{json.dumps(my_element, indent=4)}"
            )
            return False
        else:
            type_ok = False
            ...
            if not type_ok:
                self.logger.error(
                    f"Value of **{key_name}** needs to be one "
                    f"of these types: {types}\nBut got {type(my_element[key_name])}\nValue belongs to:\n{my_element}"
                )
                return False
            ...
            if values is not None:
                ...
                if not value_ok:
                    self.logger.error(
                        f"Value of **{key_name}** needs to be one "
                        f"of these specific values: {values}\nValue belongs to:\n{my_element}"
                    )
                    return False
            return True
    ...
    def _check_pipe_element(self, pe):
        ...
        if "script" in pe:
            ...
            ret = False
        if not ret:
            self.logger.error(f"The error above occured in:\n{json.dumps(pe, indent=4)}")
        return ret
    ...
    def _check_connections(self, pipe_elements):
        ...
        for pe in pipe_elements:
            if pe["peN"] in pe_map:
                self.logger.error(
                    "Multiple pipe elements have the same peN: {}\nElements are:\n{}\n{}".format(
                        pe["peN"], json.dumps(pe_map[pe["peN"]], indent=4), json.dumps(pe, indent=4)
                    )
                )
                ret = False
            else:
            ...
            if pe["peOut"] is not None:
                for pe_out in pe["peOut"]:
                    ...
                    if pe_out not in pe_map:
                        self.logger.error(
                            ("peN: {} points to peOut: {} that does not exist! See element:\n{}").format(
        ...
        for pe in pipe_elements:
            if pe["peN"] not in pe_out_set:
                self.logger.warning("No element is connected to peN: {}".format(pe["peN"]))
        return ret
