...
def __get_next_two_d_anno(db_man, user_id, at, max_amount):
    #################### get locked priority ########################
    annos = db_man.get_two_d_annotations_by_state(at.idx, state.Anno.LOCKED_PRIORITY, user_id, max_amount)
    if len(annos) > 0:
        for anno in annos:
    ...
    #################### get view locked ########################
    annos = db_man.get_two_d_annotations_by_state(at.idx, state.Anno.LOCKED, user_id, 0)
    if len(annos) > 0:
        ...
        if len(annos) < max_amount:
            ...
            tempannos = db_man.get_image_annotation_by_sim_class(at.idx, sim_class, (max_amount - len(annos)))
...
def __get_next_image_anno(db_man, user_id, at, max_amount):
    ...
    #################### get locked priority ########################
    annos = db_man.get_image_annotations_by_state(at.idx, state.Anno.LOCKED_PRIORITY, user_id, max_amount)
    if len(annos) > 0:
        for anno in annos:
            anno.timestamp_lock = datetime.now()
            db_man.save_obj(anno)
        ...
        return image_serialize.mia_json
        #################### get view locked ########################
    annos = db_man.get_image_annotations_by_state(at.idx, state.Anno.LOCKED, user_id, 0)
    if len(annos) > 0:
        while len(annos) > max_amount:
            annos[len(annos) - 1].state = state.Anno.UNLOCKED
            db_man.save_obj(annos[len(annos) - 1])
