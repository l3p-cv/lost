...
class Report:
    ...
    def __get_annos_per_label(self):
        data = []
        labels = []
        two_d_anno_counts = self.dbm.count_two_d_annos_per_label_by_pe(
            self.pe_id, self.user_id, self.iteration, self.date_from, self.date_to, exclude_current_iteration=True
        )
    ...
    def __get_annos_per_day(self):
        data = []
        labels = []
        two_d_anno_counts = self.dbm.count_two_d_annos_per_day(
            self.pe_id, self.user_id, self.iteration, self.date_from, self.date_to, exclude_current_iteration=True
        )
        for row in two_d_anno_counts:
            data.append(row[0])
            labels.append(f"{row[3]}-{row[2]}-{row[1]}")
        return data, labels
