...
def get_client(user):
    if config.worker_management == "dynamic":
        ...
        client = Client(f"{config.scheduler_ip}:{config.scheduler_port}")
    return client
...
class DaskSessionMan:
    ...
    def create_user_cluster(self, user):
        if self.lostconfig.worker_management == "dynamic":
            ...
            self.session[_user_key(user)] = {"client": client, "timestamp": datetime.utcnow()}
        else:
    ...
    def shutdown_timed_out_clients(self, logger=None):
        ...
        for key in keys:
            ...
            if logger is not None:
                logger.info(f"Release client cluster for user: {key}")
                logger.info(f"Present time: {present.isoformat()}")
                logger.info(f"Timeout time: {timeout_time.isoformat()}")
...
class PipeProjectPackageMan:
    ...
    def prepare_import(self, client, pp_path, script_name, logger=None):
        if logger is not None:
            logger.info(f"pp_path: {pp_path}")
        timestamp = datetime.now().strftime("%m%d%Y%H%M%S")
        packed_pp_path = self.fm.get_packed_pipe_path(f"{os.path.basename(pp_path)}.zip", timestamp)
        if logger is not None:
            logger.info(f"packed_pp_path: {packed_pp_path}")
        new_import_name = exec_utils.get_import_name_by_script(script_name, timestamp)
        update, import_name = self._get_import_and_update(client, pp_path, new_import_name)
        if update:
