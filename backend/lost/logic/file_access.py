...
def create_user_default_fs(dbm, user, group_id):
    if user.has_role(roles.ADMINISTRATOR) or user.has_role(roles.DESIGNER):
        ...
        if fs_db is None:
            fs_db = model.FileSystem(
                name=user.user_name,
                group_id=group_id,
                connection=default.connection,
                root_path=user_root_path,
                fs_type=default.fs_type,
...
class UserFileAccess:
    ...
    def load_anno_img(self, db_img):
        ...
        user = self.user
        if (
            anno_task.pipe_element.pipe.manager_id == user.idx
            or user.has_role(roles.ADMINISTRATOR)
            or anno_task.group_id in [g.group_id for g in user.groups]
        ):
