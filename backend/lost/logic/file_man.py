...
def chonkyfy(fs_list, root, fs):
    ...
    for idx in range(len(root.split("/"))):
        ...
        if head != "":
            folder_chain.insert(0, {"id": os.path.join(body, head), "name": head})
    for el in fs_list:
        res = {"id": el["name"], "name": os.path.basename(el["name"])}
        try:
...
class AppFileMan:
    ...
    def get_debug_path(self, pipe_element):
        ...
        pe_i_path = join(i_path, f"i-{pipe_element.idx!s}")
...
class FileMan:
    ...
    def get_pipe_log_path(self, pipe_id):
        ...
        if not self.fs.exists(base_path):
            self.fs.mkdirs(base_path)
        return os.path.join(base_path, f"p-{pipe_id}.log")
    ...
    def get_pipe_context_path(self, pe=None, pipe=None):
        ...
        """
        if pe is not None:
            pipe_context = join(self.root_path, INSTANCE_ROOT_PATH, f"p-{pe.pipe_id}")
            if not self.fs.exists(pipe_context):
                self.fs.mkdirs(pipe_context)
            return pipe_context

        elif pipe is not None:
            pipe_context = join(self.root_path, INSTANCE_ROOT_PATH, f"p-{pipe.idx}")
            if not self.fs.exists(pipe_context):
                self.fs.mkdirs(pipe_context)
            return pipe_context
        else:
    ...
    def _path_to_dict(self, path):
        ...
        d["type"] = "directory"
        d["children"] = [{"name": os.path.basename(p), "type": "directory"} for p in self.fs.ls(path)]
        return d

    def get_sia_history_path(self, annotask):
        """str: get absolute sia_history_path"""
        sia_hist_file = os.path.join(self.root_path, SIA_HISTORY_PATH, f"{annotask.idx}_{annotask.name}.json")
    ...
    def rm_sia_history_path(self, annotask):
        ...
        backup_path = os.path.join(
            self.root_path, SIA_HISTORY_BACKUP_PATH, f"{datetime.now()}_{annotask.idx}_{annotask.name}.json"
        )
        if self.fs.exists(path):
