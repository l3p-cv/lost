...
class PersonalStats:
    ...
    def get_annotation_stats(self):
        ...
        history_month = []
        for row in self.dbm.get_number_twod_annos_in_time_group_by(
    ...
    def get_anno_times(self):
        ...
        try:
            stats["today"] = (
                f"{self.dbm.mean_anno_time_by_user(self.user_id, start=self.yesterday, end=self.today)[0]:.2f}"
            )
        except:
            stats["today"] = f"{0:.2f}"
        ...
        history_month = []
        for row in self.dbm.mean_anno_time_by_user_grouped_by(
            self.user_id, start=self.last_month, end=self.today, group_by="day"
        ):
            history_month.append(row[0])
        try:
    ...
    def get_annotasks(self):
        ...
        for row in self.dbm.get_processed_anno_tasks_in_time(self.user_id, start=self.last_week, end=self.today):
            history_week.append(row[0])
        history_month = []
        for row in self.dbm.get_processed_anno_tasks_in_time(
            self.user_id, start=self.last_month, end=self.today, group_by="day"
        ):
            history_month.append(row[0])
        try:
    ...
    def get_processed_images(self):
        ...
        history_month = []
        for row in self.dbm.get_number_image_annos_in_time_group_by(
