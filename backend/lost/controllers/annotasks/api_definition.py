...
from lost.controllers.label.api_definition import label_leaf

amount_per_label = api.model(
    "Amount per Label List",
    {
...
storage_settings = api.model(
    "Storage Settings",
    {
        "datasetId": fields.Integer(
            readOnly=True, description="The dataset ID of the anno task", attribute="dataset_id"
        ),
    },
)

anno_task_export = api.model(
    "Anno Task export",
    {
        ...
        "annoTaskId": fields.Integer(
            readOnly=True, description="The identifier of the anno task", attribute="anno_task_id"
        ),
        "progress": fields.Float(readOnly=True, description="The identifier of the anno task", attribute="progress"),
        "annotaskProgress": fields.Float(
            readOnly=True, description="The identifier of the anno task", attribute="anno_task_progress"
        ),
...
anno_task_export_list = api.model(
    "Anno Task Export List",
    {"annoTasksExports": fields.List(fields.Nested(anno_task_export), attribute="anno_task_exports")},
)

anno_task_export_download = api.model(
    "Anno Task Export List", {"export": fields.String(readOnly=True, description="Binary File Data", format="binary")}
)

anno_task_filter_lables = api.model(
    "Anno Task Filter Labels",
    {"export": fields.List(fields.Integer, readOnly=True, description="List of Filter Label IDs")},
)

review_image = api.model(
    "Anno Task Filter Labels",
    {
        ...
        "annotationId": fields.Integer(
            readOnly=True, description="List of Filter Label IDs", attribute="annotation_id"
        ),
        "annotationName": fields.String(
            readOnly=True, description="List of Filter Label IDs", attribute="annotation_name"
        ),
    },
)

review_images = api.model(
    "Images for annotask Review",
    {"images": fields.Nested(review_image, description="List of Review Images", attribute="images")},
)

review_options = api.model(
    "Images for annotask Review",
    {
