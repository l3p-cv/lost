...
from lost.controllers.annotasks.api_definition import (
    anno_task,
...
from lost.controllers.annotasks.parsers import (
...
class Available(Resource):
    ...
    def get(self):
        ...
        if not user.has_role(roles.ANNOTATOR):
            ...
            dbm.close_session()
            return {"anno_tasks": annotask_list, "pages": total_pages}
...
class GetAnnoTaskStorageSettings(Resource):
    ...
    def get(self, annotask_id):
        ...
        return {"dataset_id": anno_task.dataset_id}
    ...
    def get(self, annotask_id):
        ...
        if labels is not None:
            ...
            if len(search_labels) == 0:
                db_result = dbm.get_images_without_annotations([annotask_id], search_str, annotated_only=annotated_only)
                found_images = [
                    {
                        "image_id": entry.idx,
                        "image_path": entry.img_path,
