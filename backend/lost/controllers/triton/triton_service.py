...
logger = logging.getLogger("lost.controllers.triton")
...
def process_polygon(polygon, img_id, img_width, img_height, class_id=None, labels=None):
    ...
    for point in polygon:
        x, y = point
        data.append({"x": x / img_width, "y": y / img_height})

    lost_class = (
        next((item for item in labels["labels"] if int(item.get("externalId")) == int(class_id)), None)
...
class TritonService:
    ...
    def get_available_models(self):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to fetch model list: {e!s}")
    ...
    def check_server_liveness(self):
        ...
        try:
            ...
            raise RuntimeError(f"Server liveness check failed: {e!s}")
        ...
        if model.model_type == InferenceModelType.YOLO:
            ...
            if model.task_type == InferenceModelTaskType.DETECTION:
                ...
                for det in detections:
                    processed_detection = process_detection(
                        det, img_height, img_width, inference_request.image_id, labels
                    )
                ...
                response = client.infer(
                    model_name=model_name, inputs=[input], outputs=[polygon_output, class_ids_output]
                )
                ...
                for polygon, class_id in zip(polygons, class_ids):
                    processed_polygon = process_polygon(
                        polygon, inference_request.image_id, img_width, img_height, class_id, labels
                    )
            ...
            points = (
                np.array(
                    [[[round(p.x * img_width), round(p.y * img_height)] for p in inference_request.prompts.points]],
                    dtype=np.int64,
                )
                if inference_request.prompts.points
                else None
            )

            labels = (
                np.array(
                    [[1 if p.label == "positive" else 0 for p in inference_request.prompts.points]], dtype=np.int64
                )
                if inference_request.prompts.points
                else None
            )
            ...
            bbox = (
                ...
                if inference_request.prompts.bbox
                else None
            )
