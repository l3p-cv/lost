...
def report_script_err(pipe_element, task, dbm, msg):
    ...
    if pipe_element.error_msg is None:
        ...
        pipe_element.error_msg += str(msg)
    debug_info = f"\nPipeElementID = {pipe_element.idx}"
...
class Script(pe_base.Element):
    ...
    def __init__(self, pe_id=None):
        if pe_id is None:
            ...
            parser.add_argument("--idx", nargs="?", action="store", help="Id of related pipeline element.")
        ...
        self._logger = log.get_stream_logger(os.path.basename(pe.script.path), self._log_stream)
    ...
    def _run(self, ret_success=False):
        try:
            self.main()
            self.i_am_done()
            success = f"PipeElementID: {self._pipe_element.idx}, Successfully executed script: {self._pipe_element.script.name}"
    ...
    def __str__(self):
        my_str = (
            f"I am a Script.\nMy name is: {self._pipe_element.script.name}\nPipeElementID: {self._pipe_element.idx}"
        )
        return my_str
    ...
    def get_path(self, file_name, context="instance"):
        ...
        if context == "instance":
            ...
            raise Exception(f"Unknown context: {context}. Should be *instance* or *pipe*!")
        return path
