...
def trans_boxes_to(boxes, convert_to="minmax"):
    ...
    if convert_to == "minmax":
        ...
        raise Exception(f"Unknown convert_to format: {convert_to}")
...
def to_abs(annos, types, img_size):
    ...
    for twod, t in zip(annos, types):
        if t == "bbox":
            new.append([twod[0] * w, twod[1] * h, twod[2] * w, twod[3] * h])
            ...
            raise Exception(f"Unknown annotation type: {t}")
...
def calc_box_for_anno(annos, types, point_padding=0.05):
    ...
    for anno, t in zip(annos, types):
        if np.max(anno) > 1.0:
            raise ValueError(f"Annotation are expected to be in relative format! But found: {anno}")
        if t == "bbox":
    ...
    if annos:
        ...
        for anno, t in zip(annos, types):
            if t == "bbox":
                ...
                rr, cc = polygon_perimeter([ymin, ymin, ymax, ymax], [xmin, xmax, xmax, xmin], shape=img.shape)
            elif t == "polygon":
                anno = to_abs([anno], [t], (img_w, img_h))[0]
                anno = np.array(anno)
                rr, cc = polygon_perimeter(anno[:, 1].tolist(), anno[:, 0].tolist(), shape=img.shape)
            elif t == "point":
                ...
                for i, point in enumerate(anno):
                    if i >= (len(anno) - 1):
                        break
                    rr, cc = line(point[1], point[0], anno[i + 1][1], anno[i + 1][0])
                    img[rr, cc] = color
            else:
