...
class ImgBlacklist:
    ...
    def add(self, imgs):
        ...
        if type(imgs) != list and type(imgs) != set:
            self.my_script.logger.warning(f"Lists should be used as argument for add method! Not {type(imgs)}")
    ...
    def remove_item(self, item):
        ...
        try:
            ...
            self.my_script.logger.warning(
                f"Tried to remove item from blacklist, but {item} is not present in blacklist"
            )
