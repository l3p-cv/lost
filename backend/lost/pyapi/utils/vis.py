...
def boxes(script, img_anno, figsize=(15, 15), fontsize=15, label_offset=(0, 15)):
    ...
    return _boxes(
        script.get_abs_path(img_anno.img_path),
        img_anno.bbox_vec,
        lbls,
        figsize=figsize,
        fontsize=fontsize,
            ...
            if box.size > 0:
                ...
                if i == 0 and box[0] == frame_n:
                    ...
                    bbox = patches.Rectangle(
                        (x - w / 2, y - h / 2), w, h, linewidth=linewidth, edgecolor=dot_color, facecolor="none"
                    )
                    ax.add_patch(bbox)
                # print('dot_color',dot_color)
                # dot = patches.Circle((x, y), radius=dot_radius-(dot_radius/dots)*i, color=dot_color)
                dot = patches.Circle(
                    (x, y), radius=dot_radius * 1.0 / (frame_n + 1 - box[0]), color=dot_color, alpha=0.8
                )
