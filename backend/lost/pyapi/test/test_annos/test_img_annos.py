...
def full_img_anno():
    ...
    twod_anno = model.TwoDAnno(
        data=json.dumps({"x": REF_BBOX[0], "y": REF_BBOX[1], "w": REF_BBOX[2], "h": REF_BBOX[3]}),
    ...
    twod_anno2 = model.TwoDAnno(data=json.dumps({"x": REF_POINT[0], "y": REF_POINT[1]}), dtype=dtype.TwoDAnno.POINT)
...
def add_local_fs(dbm, fs_name):
    test_user = testils.get_user(dbm)
    fs = model.FileSystem(
        group_id=test_user.groups[0].idx,
