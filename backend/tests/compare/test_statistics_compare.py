"""Golden-snapshot comparison tests for the statistics namespace."""
...
from tests.compare.statistics_specs import get_active_statistics_specs, RouteSpec


def _run_statistics_spec(client, auth_headers, spec: RouteSpec, record: bool):
    req = spec.request
    ...
    gpath = f"statistics/{spec.name}.json"
...
_ACTIVE_SPECS = get_active_statistics_specs()


@pytest.mark.parametrize("spec", _ACTIVE_SPECS, ids=[s.name for s in _ACTIVE_SPECS])
def test_statistics_route(client, auth_headers, record, spec: RouteSpec):
    _run_statistics_spec(client, auth_headers, spec, record=record)
