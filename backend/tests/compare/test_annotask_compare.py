...
- Idempotent mutate-then-GET (follow-up, no setup/cleanup): capture → save → follow-up → compare
...
def _run_annotask_spec(client, auth_headers, dbm, spec: RouteSpec, record: bool):
    ...
    Supports:
    - Simple GET/POST (no setup, no follow-up)
    - Idempotent mutate-then-GET (follow-up, no setup/cleanup)
    - Reversible mutations (setup + follow-up + cleanup)
    """
    context: dict = {}
